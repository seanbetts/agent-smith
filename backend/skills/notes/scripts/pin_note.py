...
def pin_note_database(note_id: str, pinned: bool) -> dict:
    if SessionLocal is None or NotesService is None:
        raise RuntimeError("Database dependencies are unavailable")

    db = SessionLocal()
    try:
        note = NotesService.update_pinned(db, uuid.UUID(note_id), pinned)
...
def main() -> None:
    ...
    try:
        ...
        result = pin_note_database(args.note_id, parse_bool(args.pinned))
